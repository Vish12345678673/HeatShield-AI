  ...
  head: () => ({
    meta: [
      ...
      },
      {
        name: "description",
        content:
          "GeoAI ranks where intervention can protect the most people — a live leaderboard of priority heat-mitigation zones.",
      },
      {
        property: "og:title",
        content: "City Heat Planner — HeatShield AI",
      },
      {
        property: "og:description",
        content:
          "GeoAI ranks where intervention can protect the most people.",
      },
    ],
  }),
  component: CityPage,
...
function CityPage() {
  ...
  /*
   * Everything on this page is derived from the active metro.
   * This is important because changing the metro must change both
   * the ranking and the geographic zones shown on the map.
   */
  const zones = useMemo(
    () => zonesFor(metro),
    [metro],
  ...
  /*
   * Reset the selected zone whenever the metro changes.
   * Otherwise a zone ID from the previous metro can remain selected.
   */
  useEffect(() => {
    setSelected(ranking[0]?.id ?? null);
  }, [metro.id, ranking]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_360px]">
        ...
        {/* Right-side zone preview and details */}
        <div className="space-y-4">
          <ChartCard
            title="Zone Preview"
            subtitle={
          ...
          >
            <GoogleHeatMap
              /*
               * Force a clean Google Maps instance whenever
               * the active metro changes.
               */
              key={metro.id}
              metro={metro}
              mode="2d"
          ...
          {selectedZone ? (
            <GlassCard className="p-5">
              <div className="flex items-center justify-between gap-3">
                ...
                <ToneBadge
                  tone={riskBand(selectedZone.risk).tone}
                >
              ...
              <div className="mt-4 space-y-3 text-xs">
                ...
                <div>
                  <div className="mb-1 flex justify-between text-muted-foreground">
                    ...
                    <span className="font-semibold text-foreground">
                      {selectedZone.populationPct}%
                    </span>
                  </div>

                  <ProgressTrack
                    value={selectedZone.populationPct}
                    tone="cyan"
                  />
                </div>
      ...
      <div
        ...
        style={{
          background: `radial-gradient(
            ...
            color-mix(
              in oklab,
              ${isFirst ? "var(--peach)" : toneVar.cyan}
              ${isFirst ? 18 : 10}%,
              transparent
            ),
            transparent 60%
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {zone.interventions.map((name) => {
            ...
            return (
              <span
                key={name}
                className="flex items-center gap-1 rounded-full bg-muted/60 px-2 py-1 text-[10px] text-muted-foreground"
              >
